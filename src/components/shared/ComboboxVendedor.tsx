...
export function ComboboxVendedor({
  options,
  value,
  onChange,
  onCreate,
  placeholder = 'Buscar o agregar vendedor...',
  disabled,
}: {
  options: Vendedor[]
  value: string
  onChange: (id: string, v: Vendedor) => void
  onCreate: (nombre: string) => Promise<Vendedor>
  placeholder?: string
  disabled?: boolean
}) {
        ...
        <div className="absolute top-full left-0 z-50 mt-1 max-h-56 w-full overflow-auto rounded-xl border border-[var(--line)] bg-[var(--surface-strong)] shadow-lg">
          ...
          <ul className="max-h-40 overflow-auto py-1">
            ...
            {filtered.length === 0 && !filter && (
              <li className="px-3 py-2 text-sm text-[var(--sea-ink-soft)]">Sin vendedores. Escriba y agregue uno.</li>
            )}
          </ul>
        </div>
      )}
